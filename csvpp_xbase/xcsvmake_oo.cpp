...
//  if (delFlg){DeleteFile(chgFilePath);}
  ...
  int len = strlen(chgFilePath);
  if (len == 0){return;}
  for(int i=0;i<len;i++){
    if (chgFilePath[i] == '\\'){chgFilePath[i] = '/';}
  }
...
//  Variant objDocument       = objDesktop.OleFunction("loadComponentFromURL",fileName, "_blank", 0, loadComponentFromURL_opt);
  ...
  oWindow.OleFunction("setVisible",visual_flg);
  ...
  ARY_LOOP(s_str,sChar,sheet_fp)
    ...
    oWindow_csv.OleFunction("setVisible",visual_flg);
...
//  MyStruct_read_0.OlePropertySet("Value",true);
//  MyStruct_read_0.OlePropertySet("Value",false);
  MyStruct_read_0.OlePropertySet("Name","Minimized");
  MyStruct_read_0.OlePropertySet("Value",true);
...
//    View.OleFunction("setActiveSheet", Sheet);
    View.OleFunction("Select", Sheet);
//    Sheet =  View.OleFunction("getActiveSheet");
